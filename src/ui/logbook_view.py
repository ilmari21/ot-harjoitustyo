...
class LogbookView:
    ...
    def _handle_logout(self):
        """Method responsible for handling the logging out of the user."""
    ...
    def _update_added_flights(self):
        ...
        for flight in flights:
            ...
            formatted_elapsed_time = self._logbook_service.format_elapsed_time(
                flight.elapsed_time)
            flights_tree.insert("", "end", values=(
        ...
        ttk.Label(
            master=self._frame,
            text=f"Total flights: {stats['total_flights']}    Total flight time: {stats['formatted_total_time']}",
            font=("Segoe UI", 10)
        ).grid(row=len(flights) + 1, column=0, columnspan=2, padx=10, pady=2)

    def _create_flights_tree(self):
        """Creates a table of added flights.

        Returns:
            A ttk.Treeview object consisting of a table of flights added by the user.
        """
